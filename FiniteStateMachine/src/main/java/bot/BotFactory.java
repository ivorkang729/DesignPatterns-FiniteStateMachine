...
public class BotFactory {
    ...
    public static Bot createBot(WaterballCommunity waterballCommunity, String startedTime, int initialCommandQuota) {
		...
		State knowledgeKingState = new KnowledgeKingState(
				bot, 
				// Parent --> SubState
				(Context c, State s) -> {},
				(Context c, State s) -> {});
		...
		Transition loginTransition = new Transition(
				...
				(Context c, State s, Event e) -> waterballCommunity.getLoggedInMemberCount() >= 10 ,  // Guard always returns true for simplicity
				(Context c, State s, Event e) -> {}, 	// Action to execute on transition
		...
		Transition goBroadcastingTransition = new Transition(
				...
				(Context c, State s, Event e) -> {return true;} , // Guard
				(Context c, State s, Event e) -> {}, // Action
		...
		Transition stopBroadcastingTransition = new Transition(
				...
				// Guard
				(Context c, State s, Event e) -> {return true;} ,
				// Action 
				(Context c, State s, Event e) -> {
		...
		// 設置初始狀態為 NormalState
		context.setCurrentState(normalState);
