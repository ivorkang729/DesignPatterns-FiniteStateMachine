...
public class IntegrationTest06 extends BaseTest {
	...
	public void test() throws IOException {
		...
		waterballCommunity.newMessage("1", "測試正常對話，現在 quota 應該剩下 15。", new String[] {"bot"});
		
		// 第二輪：知識王測試（quota: 15 -> 10）
		waterballCommunity.newMessage("1", "king", new String[] {"bot"});
		waterballCommunity.newMessage("1", "king-stop", new String[] {"bot"});
		waterballCommunity.newMessage("1", "測試正常對話，現在 quota 應該剩下 10。", new String[] {"bot"});
		
		// 第三輪：知識王測試（quota: 10 -> 5）
		waterballCommunity.newMessage("1", "king", new String[] {"bot"});
		waterballCommunity.newMessage("1", "king-stop", new String[] {"bot"});
		waterballCommunity.newMessage("1", "測試正常對話，現在 quota 應該剩下 5。", new String[] {"bot"});
		
		// 錄音測試（quota: 5 -> 2）
		waterballCommunity.newMessage("1", "record", new String[] {"bot"});
		waterballCommunity.goBroadcasting(user1);
		waterballCommunity.speak(user1, "測試演講。");
		waterballCommunity.newMessage("1", "stop-recording", new String[] {"bot"});
		waterballCommunity.newMessage("1", "測試正常對話，現在 quota 應該剩下 2。", new String[] {"bot"});
		...
		// 嘗試知識王（quota 不足，應該失敗）
		waterballCommunity.newMessage("1", "king", new String[] {"bot"});
		waterballCommunity.newMessage("1", "測試正常對話，現在 quota 應該剩下 2，不能玩 Knowledge King。", new String[] {"bot"});
